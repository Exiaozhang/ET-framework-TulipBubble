...
namespace ETHotfix
{
    ...
    {
        protected override async ETTask Run(Session session, C2G_StartMatch_Req request, G2C_StartMatch_Back response,
            Action reply)
        {
            try
            {
                Log.Debug("Player Start Match");
                //验证Session
                ...
                RoomConfig roomConfig = GateHelper.GetTulipBubbleRoomConfig(RoomLevel.Lv100);
                UserInfo userInfo = await Game.Scene.GetComponent<DBProxyComponent>().Query<UserInfo>(user.UserID);

                if (userInfo.Rank < roomConfig.MinThreshold)
                {
